...
| Xem quán theo danh mục | `stores` + `system_categories` | Filter quán theo danh mục (Com, Phở/Bún, Trà sữa...) |
...
| Chức năng | Firestore Collections/Fields | Mô tả |
|---|---|---|
| Quản lý danh mục trang chủ | `system_categories` | CRUD danh mục hiển thị trên app (Com, Phở/Bún, Trà sữa...), sắp xếp theo `order` |
...
| Chức năng | Firestore Collections/Fields | Mô tả |
|---|---|---|
| Duyệt yêu cầu rút tiền | `transactions` → filter `type == "withdrawal" && status == "pending"` | Xem và duyệt/r拒否 yêu cầu rút tiền từ Merchant/Driver |
