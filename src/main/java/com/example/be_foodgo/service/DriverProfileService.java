...
public class DriverProfileService {
    ...
    public void updateFCMToken(String driverId, String fcmToken) {
        try {
            Map<String, Object> updates = new HashMap<>();
            updates.put("fcmToken", fcmToken);
            updates.put("updatedAt", Instant.now());
            driverRepository.updateDriverProfileFields(driverId, updates);
            log.info("Da cap nhat FCM token cho tai xe [{}]", driverId);
        } catch (Exception e) {
            log.error("Loi khi cap nhat FCM token cho tai xe [{}]: {}", driverId, e.getMessage());
        }
    }
