...
import com.example.be_foodgo.dto.driver.DriverFCMTokenRequest;
...
public class DriverController extends BaseDriverController {
    ...
    @Operation(
            ...
            description = "Tai xe gui FCM token de nhan push notification tu he thong."
    )
    @ApiResponses(value = {
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    responseCode = "200",
                    description = "Cap nhat FCM token thanh cong"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    responseCode = "401",
    ...
    public ResponseEntity<?> updateFCMToken(
            HttpServletRequest httpRequest,
            @Valid @RequestBody DriverFCMTokenRequest request) {
        ResponseHolder holder = layUserIdHoacTraLoiLoi(httpRequest);
        if (holder.isAuthError) {
            return ResponseEntity.status(401).body(holder.errorResponse);
        }
        ...
        try {
            driverProfileService.updateFCMToken(holder.userId, request.getFcmToken());
            return ResponseEntity.ok(ApiResponse.thatSuccess(null, "Cap nhat FCM token thanh cong."));
        } catch (Exception e) {
            log.error("Loi khi cap nhat FCM token: {}", e.getMessage());
            return ResponseEntity.internalServerError().body(
